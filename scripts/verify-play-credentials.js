...
import {
  getSubscription,
  PlayBillingError,
} from "../src/services/playBillingService.js";
...
try {
  ...
  if (error.status === 404) {
    ...
    console.log(
      "(404 = authenticated fine, the probe token simply does not exist.)",
    );
    process.exit(0);
  }
  ...
  if (error.status === 403) {
    console.error(
      "Credentials authenticate, but Play Console has not granted access.",
    );
    console.error(
      "Check: Play Console > Users & permissions > the service account has",
    );
    console.error(
      "'View financial data' on this app. Propagation takes 24-48h.",
    );
    process.exit(1);
  }
  ...
  if (error.status === 401) {
    console.error(
      "Authentication failed. The service account key is wrong or the",
    );
